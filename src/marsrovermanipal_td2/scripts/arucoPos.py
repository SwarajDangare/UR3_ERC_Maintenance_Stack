...
class arucoPos:
    ...
    def callback(self,data):
        ...
        for i in range(len(data.transforms)):
            ...
            self.ls.waitForTransform("fiducial_" + str(frame.fiducial_id),"world",rospy.Time(0), rospy.Duration(15.0))
            self.aruco[frame.fiducial_id] = self.ls.lookupTransform("world","fiducial_" + str(frame.fiducial_id), rospy.Time(0))
            self.br.sendTransform(self.aruco[frame.fiducial_id][0], self.aruco[frame.fiducial_id][1], rospy.Time(0), "Aruco_" + str(frame.fiducial_id), "world")
                #br.sendTransform(trans, rot, now, "camera_link", "world")
